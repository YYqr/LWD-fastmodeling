"""
Kernel of lwdfast, calculates the wavenumber-domain electromagnetic
...
try:
    from lwdfast import cpp_backend as _cpp_backend
except ImportError:  # The compiled extension is optional.
    _cpp_backend = None
...
def _wavenumber_numba(zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH,
    ...
    *Quadrature-with-Extrapolation* in :mod:`lwdfast.transform` are set-up with
    ...
    :mod:`lwdfast.transform`.  Consult the modelling routines in
    :mod:`lwdfast.model` for a description of the input and output parameters.
...
def wavenumber(zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH, zetaV,
    ...
    """Calculate wavenumber kernels with C++ assembly when available."""
    if _cpp_backend is not None and _cpp_backend.AVAILABLE:
        PTM, PTE = greenfct(
            zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH, zetaV,
            lambd, ab, xdirect, msrc, mrec,
        )
        return _cpp_backend.wavenumber_from_green(PTM, PTE, lambd, ab, mrec)
    return _wavenumber_numba(
        zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH, zetaV,
        lambd, ab, xdirect, msrc, mrec,
    )
...
@nb.njit(**_numba_setting)
def _greenfct_numba(zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH, zetaV,
                    lambd, ab, xdirect, msrc, mrec):
...
def _greenfct_cpp(zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH,
                  zetaV, lambd, ab, xdirect, msrc, mrec):
    """Layered Green functions using vectorized algebra and C++ recursions."""
    nfreq, nlayer = etaH.shape
    noff, nlambda = lambd.shape
    ...
    GTM = np.zeros((nfreq, noff, nlambda), dtype=etaH.dtype)
    GTE = np.zeros_like(GTM)
    gamTM = None
    gamTE = None
    lambda_squared = lambd[None, :, None, :]**2

    for TM in (True, False):
        if TM and ab in (16, 26):
            continue
        if not TM and ab in (13, 23, 31, 32, 33, 34, 35):
            continue
        ...
        Gam = np.sqrt(
            (e_zH/e_zV)[:, None, :, None]*lambda_squared
            + (z_eH*e_zH)[:, None, :, None]
        )
        lrecGam = Gam[:, :, lrec, :]

        if nlayer > 1:
            Rp, Rm = _cpp_backend.reflections(depth, e_zH, Gam, lrec, lsrc)
            Wu = np.zeros_like(lrecGam)
            Wd = np.zeros_like(lrecGam)
            if lrec != nlayer-1:
                Wu = np.exp(-lrecGam*(depth[lrec+1]-zrec))
            if lrec != 0:
                Wd = np.exp(-lrecGam*(zrec-depth[lrec]))
            Pu, Pd = _cpp_backend.fields(
        ...
        if lsrc == lrec:
            ...
            thickness = 0 if lrec == nlayer-1 else depth[lrec+1]-depth[lrec]
            fexp = np.exp(-lrecGam*thickness)
            pmw = -1 if TM and ab in (
                11, 12, 13, 21, 22, 23, 14, 24, 15, 25,
            ) else 1
            if lrec < lsrc:
                green = Pu*(Wu+pmw*Rm[:, :, 0, :]*fexp*Wd)
            else:
                green = Pd*(
                    pmw*Wd+Rp[:, :, abs(lsrc-lrec), :]*fexp*Wu
                )
    ...
    if ab in (11, 12, 21, 22):
        GTM *= gamTM[:, :, lrec, :]/etaH[:, lrec, None, None]
        GTE *= zetaH[:, lsrc, None, None]/gamTE[:, :, lsrc, :]
    elif ab in (14, 15, 24, 25):
        fact = etaH[:, lsrc]/etaH[:, lrec]
        GTM *= fact[:, None, None]*gamTM[:, :, lrec, :]/gamTM[:, :, lsrc, :]
    elif ab in (13, 23):
        fact = etaH[:, lsrc]/etaH[:, lrec]/etaV[:, lsrc]
        GTM *= -fact[:, None, None]*gamTM[:, :, lrec, :]/gamTM[:, :, lsrc, :]
        GTE = np.zeros_like(GTM)
    elif ab in (31, 32):
        GTM /= etaV[:, lrec, None, None]
        GTE = np.zeros_like(GTM)
    elif ab in (34, 35):
        fact = etaH[:, lsrc]/etaV[:, lrec]
        GTM *= fact[:, None, None]/gamTM[:, :, lsrc, :]
        GTE = np.zeros_like(GTM)
    elif ab in (16, 26):
        fact = zetaH[:, lsrc]/zetaV[:, lsrc]
        GTE *= fact[:, None, None]/gamTE[:, :, lsrc, :]
        GTM = np.zeros_like(GTE)
    elif ab == 33:
        fact = etaH[:, lsrc]/etaV[:, lsrc]/etaV[:, lrec]
        GTM *= fact[:, None, None]/gamTM[:, :, lsrc, :]
        GTE = np.zeros_like(GTM)
    return GTM, GTE
...
def greenfct(zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH, zetaV,
             lambd, ab, xdirect, msrc, mrec):
    """Calculate layered Green functions using C++ when available."""
    if _cpp_backend is not None and _cpp_backend.AVAILABLE:
        return _cpp_backend.greenfct(
            zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH, zetaV,
            lambd, ab, xdirect, msrc, mrec,
        )
    return _greenfct_numba(
        zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH, zetaV,
        lambd, ab, xdirect, msrc, mrec,
    )
...
def _angle_factor_numpy(angle, ab, msrc, mrec):
...
def angle_factor(angle, ab, msrc, mrec):
    """Return the angle-dependent factor using C++ when available."""
    if _cpp_backend is not None and _cpp_backend.AVAILABLE:
        return _cpp_backend.angle_factor(angle, ab, msrc, mrec)
    return _angle_factor_numpy(angle, ab, msrc, mrec)


# Analytical solutions

@np.errstate(all='ignore')
def _fullspace_numpy(off, angle, zsrc, zrec, etaH, etaV, zetaH, zetaV, ab,
                     msrc, mrec):
    ...
    :mod:`lwdfast.model`. Consult these modelling routines for a description of
...
def fullspace(off, angle, zsrc, zrec, etaH, etaV, zetaH, zetaV, ab, msrc,
              mrec):
    """Return analytical full-space fields using C++ when available."""
    if _cpp_backend is not None and _cpp_backend.AVAILABLE:
        return _cpp_backend.fullspace(
            off, angle, zsrc, zrec, etaH, etaV, zetaH, zetaV,
            ab, msrc, mrec,
        )
    return _fullspace_numpy(
        off, angle, zsrc, zrec, etaH, etaV, zetaH, zetaV,
        ab, msrc, mrec,
    )
