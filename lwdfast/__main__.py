...
from lwdfast import io, model, utils
...
def main(args=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="show the lwdfast report and exit"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="show the lwdfast version and exit"
    )

    # Get command line arguments.
    args_dict = vars(parser.parse_args(args))

    # lwdfast version info.
    if args_dict.pop('version'):  # lwdfast version info.
        print(f"lwdfast v{utils.__version__}")
    ...
    # Info if not at list routine and input provided.
        ...
        print(f"{parser.description}\n=> Type `lwdfast --help` for "
              f"more info (lwdfast v{utils.__version__}).")
...
def run(args_dict):
    """Run lwdfast with provided arguments."""

    # Run lwdfast, enforce ``squeeze=False``.
    ...
    if outfile:
        info = f"Generated with <lwdfast.{fct}()> from input <{iname}>."
