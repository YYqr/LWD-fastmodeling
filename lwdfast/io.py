...
from lwdfast import utils, filters
...
def save_input(fname, data, **kwargs):
    ...
    Save the input provided to an lwdfast modelling routine on disk.
    ...
    data : dict
        ...
        for an lwdfast modelling routine.
...
def save_data(fname, data, **kwargs):
    ...
    data : ndarray
        The output from an lwdfast modelling routine.
    ...
    meta = {
        ...
        "version": f"lwdfast v{utils.__version__}",
        "shape": str(shape),
...
def load_data(fname):
    """Load results from lwdfast stored with ``save_data``.
