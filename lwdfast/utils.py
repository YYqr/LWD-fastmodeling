"""
Utilities for :mod:`lwdfast.model` such as checking input parameters.
...
# Relative imports
from lwdfast import filters, transform
...
try:
    ...
    from lwdfast.version import version as __version__
except ImportError:
    # If it was not installed, then we don't know the version. We could throw a
    # warning here, but this case *should* be rare. lwdfast should be installed
    # properly!
...
class EMArray(np.ndarray):
    ...
    --------
    >>> import numpy as np
    >>> from lwdfast.utils import EMArray
...
def check_ab(ab, verb):
    ...
    :mod:`lwdfast.model`. Consult these modelling routines for a detailed
...
def get_azm_dip(inp, iz, ninpz, intpts, isdipole, strength, name, verb):
    ...
    if verb > 2:
        ...
        # Print angles
        _prnt_min_max_val(np.rad2deg(azm), "     > azimuth [掳] : ", verb)
        _prnt_min_max_val(np.rad2deg(dip), "     > dip     [掳] : ", verb)
...
def printstartfinish(verb, inp=None, kcount=None):
    ...
    if inp:
        if verb > 1:
            ...
            print(f"\n:: lwdfast END; runtime = {ttxt} ::{ktxt}\n")
    else:
        t0 = default_timer()
        if verb > 2:
            print(f"\n:: lwdfast START  ::  v{__version__}\n")
        return t0
...
class Report(ScoobyReport):
    ...
    `lwdfast`, `sys.version`, and time/date.
    ...
    def __init__(self, add_pckg=None, ncol=3, text_width=80, sort=False):
        ...
        # Mandatory packages.
        core = ['numpy', 'scipy', 'numba', 'lwdfast', 'libdlf']
