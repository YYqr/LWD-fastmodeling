...
def _spline_matrix_cached(x_size, x_bytes, target_size, target_bytes):
    """Return a reusable not-a-knot cubic interpolation matrix.

    DLF repeatedly interpolates at identical logarithmic abscissae while only
    the complex kernel values change.  Building FITPACK splines for every
    component dominates short LWD calls.  Cubic interpolation is linear in
    the values, so the basis matrix can safely be cached and re-used.
    """
    x = np.frombuffer(x_bytes, dtype=np.float64, count=x_size)
    target = np.frombuffer(target_bytes, dtype=np.float64, count=target_size)
    matrix = sp.interpolate.make_interp_spline(
        x, np.eye(x_size, dtype=np.float64), axis=0,
    )(target)
    matrix.setflags(write=False)
    return matrix


def _cached_log_spline(values, points, targets):
    """Evaluate the legacy cubic log-spline through a cached basis matrix."""
    x = np.ascontiguousarray(np.log(np.asarray(points, dtype=np.float64)))
    target = np.ascontiguousarray(
        np.log(np.asarray(targets, dtype=np.float64))
    )
    if x.ndim != 1 or target.ndim != 1:
        out = iuSpline(x, np.asarray(values).real)(target)
        if np.iscomplexobj(values):
            out = out+1j*iuSpline(x, np.asarray(values).imag)(target)
        return out
    # Translation does not change cubic interpolation weights.  Removing the
    # common origin lets geometrically identical DLF grids share one matrix.
    origin = x[0]
    x_key = np.ascontiguousarray(x-origin)
    target_key = np.ascontiguousarray(target-origin)
    matrix = _spline_matrix_cached(
        x_key.size, x_key.tobytes(), target_key.size, target_key.tobytes(),
    )
...
def fourier_fftlog(fEM, time, freq, ftarg):
    ...
    if q == 0:  # unbiased case (q = 0)
        ...
        # first element: cos(arg) = 卤1, sin(arg) = 0
    ...
    # 脙(k) = 茫(k) k^[-dir*(q+.5)] rc^[-dir*(q-.5)]
    #      = 茫(k) (k/kc)^[-dir*(q+.5)] (kc rc)^(-dir*q) (rc/kc)^(dir*.5)
...
def dlf(signal, points, out_pts, filt, pts_per_dec, kind=None, ang_fact=None,
    ...
    def spline(values, points, int_pts):
        ...
        return _cached_log_spline(values, points, int_pts)
